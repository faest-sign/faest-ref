...
bf128_t* get_vk_128(vbb_t* vbb, unsigned int idx) {
  if (idx < FAEST_128F_LAMBDA) {
    ...
    if ((j % FAEST_128F_Nwd) == 0 || (FAEST_128F_Nwd > 6 && (j % FAEST_128F_Nwd) == 4)) {
      ...
      return get_V_k_128(vbb, index);
    } else {
      // Lhs recursive call
      bf128_t* lhs_ptr = get_vk_128(vbb, idx - FAEST_128F_Nwd * 32);
      bf128_t lhs      = *lhs_ptr;
      // Rhs recursive call
      bf128_t* rhs_ptr = get_vk_128(vbb, idx - 32);
      bf128_t rhs      = *rhs_ptr;
...
bf192_t* get_vk_192(vbb_t* vbb, unsigned int idx) {
  if (idx < FAEST_192F_LAMBDA) {
    ...
    if ((j % FAEST_192F_Nwd) == 0 || (FAEST_192F_Nwd > 6 && (j % FAEST_192F_Nwd) == 4)) {
      ...
      return get_V_k_192(vbb, i_wd + factor_192 * 32 + offset_192);
    } else {
      // Lhs recursive call
      bf192_t* lhs_ptr = get_vk_192(vbb, idx - FAEST_192F_Nwd * 32);
      bf192_t lhs      = *lhs_ptr;
      // Rhs recursive call
      bf192_t* rhs_ptr = get_vk_192(vbb, idx - 32);
      bf192_t rhs      = *rhs_ptr;
...
bf256_t* get_vk_256(vbb_t* vbb, unsigned int idx) {
  if (idx < FAEST_256F_LAMBDA) {
    ...
    if ((j % FAEST_256F_Nwd) == 0 || (FAEST_256F_Nwd > 6 && (j % FAEST_256F_Nwd) == 4)) {
      ...
      unsigned int offset_128 = idx % 128;
      return get_V_k_256(vbb, i_wd + factor_128 * 32 + offset_128);
    } else {
      // Lhs recursive call
      bf256_t* lhs_ptr = get_vk_256(vbb, idx - FAEST_256F_Nwd * 32);
      bf256_t lhs      = *lhs_ptr;
      // Rhs recursive call
      bf256_t* rhs_ptr = get_vk_256(vbb, idx - 32);
      bf256_t rhs      = *rhs_ptr;
