...
void vole_hash_128(uint8_t* h, const uint8_t* sd, const uint8_t* x, unsigned int ell) {
  ...
  bf128_load(&tmp0, r0);
  bf128_load(&tmp1, r1);
  const bf128_t h2 = bf128_add(bf128_mul(tmp0, h0), bf128_mul_64(tmp1, h1));

  bf128_load(&tmp0, r2);
  bf128_load(&tmp1, r3);
  const bf128_t h3 = bf128_add(bf128_mul(tmp0, h0), bf128_mul_64(tmp1, h1));
...
void vole_hash_192(uint8_t* h, const uint8_t* sd, const uint8_t* x, unsigned int ell) {
  ...
  bf192_load(&tmp0, r0);
  bf192_load(&tmp1, r1);
  const bf192_t h2 = bf192_add(bf192_mul(tmp0, h0), bf192_mul_64(tmp1, h1));

  bf192_load(&tmp0, r2);
  bf192_load(&tmp1, r3);
  const bf192_t h3 = bf192_add(bf192_mul(tmp0, h0), bf192_mul_64(tmp1, h1));
...
void vole_hash_256(uint8_t* h, const uint8_t* sd, const uint8_t* x, unsigned int ell) {
  ...
  bf256_load(&tmp0, r0);
  bf256_load(&tmp1, r1);
  const bf256_t h2 = bf256_add(bf256_mul(tmp0, h0), bf256_mul_64(tmp1, h1));

  bf256_load(&tmp0, r2);
  bf256_load(&tmp1, r3);
  const bf256_t h3 = bf256_add(bf256_mul(tmp0, h0), bf256_mul_64(tmp1, h1));
