...
static void aes_key_schedule_forward_128_vbb(vbb_t* vbb, bf128_t* bf_out) {
  // Step: 1 sanity check (skipped)

  for (unsigned int i = 0; i < FAEST_128F_LAMBDA; i++) {
    memcpy(bf_out + i, get_vole_v_prove_128(vbb, i), sizeof(bf128_t));
  }
  ...
  for (unsigned int j = FAEST_128F_Nwd; j < 4 * (FAEST_128F_R + 1); j++) {
    if ((j % FAEST_128F_Nwd) == 0 || (FAEST_128F_Nwd > 6 && (j % FAEST_128F_Nwd) == 4)) {
      for (unsigned int i = 0; i < 32; i++) {
        memcpy(bf_out + j * 32 + i, get_vole_v_prove_256(vbb, i_wd + i), sizeof(bf256_t));
      }
      i_wd += 32;
    } else {
      for (unsigned int i = 0; i < 32; i++) {
...
static void aes_key_schedule_backward_128_vbb(vbb_t* vbb, const bf128_t* Vk, uint8_t Mtag,
                                              uint8_t Mkey, const uint8_t* delta, bf128_t* bf_out) {
  // Step: 1
  assert(!((Mtag == 1 && Mkey == 1) || (Mkey == 1 && delta == NULL)));
  ...
  for (unsigned int j = 0; j < FAEST_128F_Ske; j++) {
    ...
    bf128_t bf_x_tilde[8];
    for (unsigned int i = 0; i < 8; i++) {
      bf_x_tilde[i] = bf128_add(*get_vole_v_prove_128(vbb, (8 * j + i) + FAEST_128F_LAMBDA),
                                Vk[iwd + 8 * c + i]);
    }
...
static void aes_key_schedule_constraints_Mkey_0_128(const uint8_t* w, vbb_t* vbb,
  ...
  aes_key_schedule_backward_128_vbb(vbb, vk, 1, 0, NULL, v_w_dash);
...
static void aes_enc_forward_128_vbb(vbb_t* vbb, unsigned int offset, const bf128_t* bf_xk,
                                    const uint8_t* in, uint8_t Mtag, uint8_t Mkey,
...
static void aes_enc_backward_128_vbb(vbb_t* vbb, unsigned int offset, const bf128_t* bf_xk,
    ...
    for (unsigned int c = 0; c <= 3; c++) {
      for (unsigned int r = 0; r <= 3; r++) {
        ...
        if (j < (FAEST_128F_R - 1)) {
          // Step: 7
          for (unsigned int i = 0; i < 8; i++) {
            memcpy(bf_x_tilde + i, get_vole_v_prove_128(vbb, ird + i + offset), sizeof(bf128_t));
          }
        } else {
          // Step: 10
          for (unsigned int i = 0; i < 8; ++i) {
...
static void aes_enc_constraints_Mkey_0_128(const uint8_t* in, const uint8_t* out, const uint8_t* w,
                                           ...
                                           const bf128_t* vk, zk_hash_128_ctx* a0_ctx,
  ...
  aes_enc_forward_128_vbb(vbb, offset, vk, in, 1, 0, NULL, vs);
  aes_enc_backward_128_1(w, k, out, s_dash);
  aes_enc_backward_128_vbb(vbb, offset, vk, 1, 0, NULL, out, vs_dash);
...
static void aes_prove_128(const uint8_t* w, vbb_t* vbb, const uint8_t* in, const uint8_t* out,
  ...
  aes_key_schedule_constraints_Mkey_0_128(w, vbb, &a0_ctx, &a1_ctx, k, vk, params);
  ...
  aes_enc_constraints_Mkey_0_128(in, out, w, vbb, offset, k, vk, &a0_ctx, &a1_ctx);
  // Step: 12 (beta == 1)
...
static uint8_t* aes_verify_128(const uint8_t* d, qbb_t* qbb, const uint8_t* chall_2,
  ...
  */
  ...
  aes_key_schedule_constraints_Mkey_1_128((bf128_t*)qbb->vole_Q_cache_RMO, delta, &b0_ctx, qk);

  // Step: 14
  aes_enc_constraints_Mkey_1_128(in, out, (bf128_t*)qbb->vole_Q_cache_RMO + FAEST_128F_Lke, qk, delta, &b0_ctx);
  // Step: 18 (beta == 1)
  faest_aligned_free(qk);

  // Step: 20+21
  uint8_t* q_tilde = malloc(FAEST_128F_LAMBDA / 8);
  zk_hash_128_finalize(q_tilde, &b0_ctx, bf128_sum_poly((bf128_t*)qbb->vole_Q_cache_RMO + FAEST_128F_L));
...
static void aes_key_schedule_forward_192_vbb(vbb_t* vbb, bf192_t* bf_out) {
  // Step: 1 sanity check (skipped)

  for (unsigned int i = 0; i < FAEST_192F_LAMBDA; i++) {
    memcpy(bf_out + i, get_vole_v_prove_192(vbb, i), sizeof(bf192_t));
  }
  ...
  for (unsigned int j = FAEST_192F_Nwd; j < 4 * (FAEST_192F_R + 1); j++) {
    if ((j % FAEST_192F_Nwd) == 0 || (FAEST_192F_Nwd > 6 && (j % FAEST_192F_Nwd) == 4)) {
      for (unsigned int i = 0; i < 32; i++) {
        memcpy(bf_out + j * 32 + i, get_vole_v_prove_192(vbb, i_wd + i), sizeof(bf192_t));
      }
      i_wd += 32;
    } else {
      for (unsigned int i = 0; i < 32; i++) {
...
static void aes_key_schedule_backward_192_vbb(vbb_t* vbb, const bf192_t* Vk, uint8_t Mtag,
                                              uint8_t Mkey, const uint8_t* delta, bf192_t* bf_out) {
  // Step: 1
  assert(!((Mtag == 1 && Mkey == 1) || (Mkey == 1 && delta == NULL)));
  ...
  for (unsigned int j = 0; j < FAEST_192F_Ske; j++) {
    ...
    bf192_t bf_x_tilde[8];
    for (unsigned int i = 0; i < 8; i++) {
      bf_x_tilde[i] = bf192_add(*get_vole_v_prove_192(vbb, (8 * j + i) + FAEST_192F_LAMBDA),
                                Vk[iwd + 8 * c + i]);
    }
...
static void aes_enc_forward_192_vbb(vbb_t* vbb, unsigned int offset, const bf192_t* bf_xk,
                                    const uint8_t* in, uint8_t Mtag, uint8_t Mkey,
                                    const uint8_t* delta, bf192_t* bf_y) {
  const bf192_t bf_delta  = delta ? bf192_load(delta) : bf192_zero();
  const bf192_t bf_factor = bf192_add(bf192_mul_bit(bf_delta, Mkey), bf192_from_bit(1 ^ Mkey));
...
static void aes_enc_backward_192_vbb(vbb_t* vbb, unsigned int offset, const bf192_t* bf_xk,
                                     uint8_t Mtag, uint8_t Mkey, const uint8_t* delta,
                                     const uint8_t* out, bf192_t* y_out) {
  // Step: 1
  const bf192_t bf_delta = delta ? bf192_load(delta) : bf192_zero();
    ...
    for (unsigned int c = 0; c <= 3; c++) {
      for (unsigned int r = 0; r <= 3; r++) {
        ...
        if (j < (FAEST_192F_R - 1)) {
          // Step: 7
          for (unsigned int i = 0; i < 8; i++) {
            memcpy(bf_x_tilde + i, get_vole_v_prove_192(vbb, ird + i + offset), sizeof(bf192_t));
          }
        } else {
          // Step: 10
          for (unsigned int i = 0; i < 8; ++i) {
...
static uint8_t* aes_verify_192(const uint8_t* d, qbb_t* qbb, const uint8_t* chall_2,
                               const uint8_t* chall_3, const uint8_t* a_tilde, const uint8_t* in,
                               const uint8_t* out, const faest_paramset_t* params) {
  /*
  const unsigned int tau = params->faest_param.tau;
  ...
  */
  ...
  aes_key_schedule_constraints_Mkey_1_192((bf192_t*)qbb->vole_Q_cache_RMO, delta, &b0_ctx, qk);

  // Step: 14
  aes_enc_constraints_Mkey_1_192(in, out, (bf192_t*)qbb->vole_Q_cache_RMO + FAEST_192F_Lke, qk, delta, &b0_ctx);

  // Step: 18
  aes_enc_constraints_Mkey_1_192(in + 16, out + 16, (bf192_t*)qbb->vole_Q_cache_RMO + FAEST_192F_Lke + FAEST_192F_Lenc, qk,
                                 delta, &b0_ctx);
  faest_aligned_free(qk);

  // Step: 20+21
  uint8_t* q_tilde = malloc(FAEST_192F_LAMBDA / 8);
  zk_hash_192_finalize(q_tilde, &b0_ctx, bf192_sum_poly((bf192_t*)qbb->vole_Q_cache_RMO + FAEST_192F_L));
  //faest_aligned_free(bf_q);
...
static void aes_key_schedule_forward_256(bf256_t* v, bf256_t* bf_out) {
  // Step: 1 sanity check (skipped)
...
static void aes_key_schedule_forward_256_vbb(vbb_t* vbb, bf256_t* bf_out) {
  // Step: 1 sanity check (skipped)

  for (unsigned int i = 0; i < FAEST_256F_LAMBDA; i++) {
    memcpy(bf_out + i, get_vole_v_prove_256(vbb, i), sizeof(bf256_t));
  }
...
static void aes_key_schedule_backward_256_vbb(vbb_t* vbb, const bf256_t* Vk, uint8_t Mtag,
                                              uint8_t Mkey, const uint8_t* delta, bf256_t* bf_out) {
  // Step: 1
  assert(!((Mtag == 1 && Mkey == 1) || (Mkey == 1 && delta == NULL)));
  ...
  for (unsigned int j = 0; j < FAEST_256F_Ske; j++) {
    ...
    bf256_t bf_x_tilde[8];
    for (unsigned int i = 0; i < 8; i++) {
      bf_x_tilde[i] = bf256_add(*get_vole_v_prove_256(vbb, (8 * j + i) + FAEST_256F_LAMBDA),
                                Vk[iwd + 8 * c + i]);
    }
...
static void aes_enc_forward_256_vbb(vbb_t* vbb, unsigned int offset, const bf256_t* bf_xk,
                                    const uint8_t* in, uint8_t Mtag, uint8_t Mkey,
                                    const uint8_t* delta, bf256_t* bf_y) {
  const bf256_t bf_delta  = delta ? bf256_load(delta) : bf256_zero();
  const bf256_t bf_factor = bf256_add(bf256_mul_bit(bf_delta, Mkey), bf256_from_bit(1 ^ Mkey));
...
static void aes_enc_backward_256_vbb(vbb_t* vbb, unsigned int offset, const bf256_t* bf_xk,
                                     uint8_t Mtag, uint8_t Mkey, const uint8_t* delta,
                                     const uint8_t* out, bf256_t* y_out) {
  // Step: 1
  const bf256_t bf_delta = delta ? bf256_load(delta) : bf256_zero();
    ...
    for (unsigned int c = 0; c <= 3; c++) {
      for (unsigned int r = 0; r <= 3; r++) {
        ...
        if (j < (FAEST_256F_R - 1)) {
          // Step: 7
          for (unsigned int i = 0; i < 8; i++) {
            memcpy(bf_x_tilde + i, get_vole_v_prove_256(vbb, ird + i + offset), sizeof(bf256_t));
          }
        } else {
          // Step: 10
          for (unsigned int i = 0; i < 8; ++i) {
...
static uint8_t* aes_verify_256(const uint8_t* d, qbb_t* qbb, const uint8_t* chall_2,
                               const uint8_t* chall_3, const uint8_t* a_tilde, const uint8_t* in,
                               const uint8_t* out, const faest_paramset_t* params) {
  /*
  const unsigned int tau = params->faest_param.tau;
  ...
  */
  ...
  for (unsigned int i = 0, col = 0; i < tau; i++) {
    ...
    for (unsigned int j = 0; j < depth; j++, ++col) {
      if (decoded_challenge[j] == 1) {
        xor_u8_array(d, qbb->vole_Q_cache_index[col], qbb->vole_Q_cache_index[col], (FAEST_256F_L + 7) / 8);
      }
    }
  }
  ...
  */
  ...
  aes_key_schedule_constraints_Mkey_1_256((bf256_t*)qbb->vole_Q_cache_RMO, delta, &b0_ctx, qk);

  // Step: 14
  aes_enc_constraints_Mkey_1_256(in, out, (bf256_t*)qbb->vole_Q_cache_RMO + FAEST_256F_Lke, qk, delta, &b0_ctx);
  // Step: 18
  aes_enc_constraints_Mkey_1_256(in + 16, out + 16, (bf256_t*)qbb->vole_Q_cache_RMO + FAEST_256F_Lke + FAEST_256F_Lenc, qk,
                                 delta, &b0_ctx);
  faest_aligned_free(qk);

  // Step: 20, 21
  uint8_t* q_tilde = malloc(FAEST_256F_LAMBDA / 8);
  zk_hash_256_finalize(q_tilde, &b0_ctx, bf256_sum_poly((bf256_t*)qbb->vole_Q_cache_RMO + FAEST_256F_L));
  //faest_aligned_free(bf_q);
  ...
  for (unsigned int j = 0; j < FAEST_EM_128F_R; j++) {
    for (unsigned int c = 0; c < FAEST_EM_128F_Nwd; c++) {
      for (unsigned int r = 0; r <= 3; r++) {
        ...
        if (j < (FAEST_EM_128F_R - 1)) {
          for (unsigned int i = 0; i < 8; i++) {
            memcpy(bf_z_tilde + i, get_vole_v_prove_128(vbb, ird + i), sizeof(bf128_t));
          }
        } else {
          for (unsigned int i = 0; i < 8; ++i) {
            // Step: 12
            bf_z_tilde[i] =
                *get_vole_v_prove_128(vbb_out, ird - 32 * FAEST_EM_128F_Nwd * (j + 1) + i);
            if (bf_x) {
              bf_z_tilde[i] = bf128_add(bf_z_tilde[i], bf_x[ird + i]);
            }
          }
...
static uint8_t* em_verify_128(const uint8_t* d, qbb_t* qbb, const uint8_t* chall_2,
                              const uint8_t* chall_3, const uint8_t* a_tilde, const uint8_t* in,
                              const uint8_t* out, const faest_paramset_t* params) {
  /*
  const unsigned int tau = params->faest_param.tau;
  ...
  */
  ...
  zk_hash_128_ctx b0_ctx;
  zk_hash_128_init(&b0_ctx, chall_2);
  em_enc_constraints_Mkey_1_128(out, x, (bf128_t*)qbb->vole_Q_cache_RMO, delta, &b0_ctx);

  uint8_t* q_tilde = malloc(FAEST_EM_128F_LAMBDA / 8);
  zk_hash_128_finalize(q_tilde, &b0_ctx, bf128_sum_poly((bf128_t*)qbb->vole_Q_cache_RMO + FAEST_EM_128F_Lenc));
  //faest_aligned_free(bf_q);
      ...
      for (unsigned int r = 0; r <= 3; r++) {
        ...
        if (j < (FAEST_EM_192F_R - 1)) {
          ...
          for (unsigned int i = 0; i < 8; ++i) {
            // Step: 12
            bf_z_tilde[i] =
                *get_vole_v_prove_192(vbb_out, ird - 32 * FAEST_EM_192F_Nwd * (j + 1) + i);
            if (bf_x) {
              bf_z_tilde[i] = bf192_add(bf_z_tilde[i], bf_x[ird + i]);
            }
          }
...
static uint8_t* em_verify_192(const uint8_t* d, qbb_t* qbb, const uint8_t* chall_2,
                              const uint8_t* chall_3, const uint8_t* a_tilde, const uint8_t* in,
                              const uint8_t* out, const faest_paramset_t* params) {
  /*
  const unsigned int tau = params->faest_param.tau;
  ...
  */
  ...
  zk_hash_192_ctx b0_ctx;
  zk_hash_192_init(&b0_ctx, chall_2);
  em_enc_constraints_Mkey_1_192(out, x, (bf192_t*)qbb->vole_Q_cache_RMO, delta, &b0_ctx);

  uint8_t* q_tilde = malloc(FAEST_EM_192F_LAMBDA / 8);
  zk_hash_192_finalize(q_tilde, &b0_ctx, bf192_sum_poly((bf192_t*)qbb->vole_Q_cache_RMO + FAEST_EM_192F_Lenc));
  //faest_aligned_free(bf_q);
  ...
  for (unsigned int j = 0; j < FAEST_EM_256F_R; j++) {
    for (unsigned int c = 0; c < FAEST_EM_256F_Nwd; c++) {
      for (unsigned int r = 0; r <= 3; r++) {
        ...
        if (j < (FAEST_EM_256F_R - 1)) {
          for (unsigned int i = 0; i < 8; i++) {
            memcpy(bf_z_tilde + i, get_vole_v_prove_256(vbb, ird + i), sizeof(bf256_t));
          }
        } else {
          for (unsigned int i = 0; i < 8; ++i) {
            // Step: 12
            bf_z_tilde[i] =
                *get_vole_v_prove_256(vbb_out, ird - 32 * FAEST_EM_256F_Nwd * (j + 1) + i);
            if (bf_x) {
              bf_z_tilde[i] = bf256_add(bf_z_tilde[i], bf_x[ird + i]);
            }
          }
...
static uint8_t* em_verify_256(const uint8_t* d, qbb_t* qbb, const uint8_t* chall_2,
                              const uint8_t* chall_3, const uint8_t* a_tilde, const uint8_t* in,
                              const uint8_t* out, const faest_paramset_t* params) {
  /*
  const unsigned int tau = params->faest_param.tau;
  ...
  */
  ...
  zk_hash_256_ctx b0_ctx;
  zk_hash_256_init(&b0_ctx, chall_2);
  em_enc_constraints_Mkey_1_256(out, x, (bf256_t*)qbb->vole_Q_cache_RMO, delta, &b0_ctx);

  uint8_t* q_tilde = malloc(FAEST_EM_256F_LAMBDA / 8);
  zk_hash_256_finalize(q_tilde, &b0_ctx, bf256_sum_poly((bf256_t*)qbb->vole_Q_cache_RMO + FAEST_EM_256F_Lenc));
  //faest_aligned_free(bf_q);
...
uint8_t* aes_verify(const uint8_t* d, qbb_t* qbb, const uint8_t* chall_2, const uint8_t* chall_3,
  ...
  case 256:
    if (params->faest_param.Lke) {
      return aes_verify_256(d, qbb, chall_2, chall_3, a_tilde, in, out, params);
    } else {
      return em_verify_256(d, qbb, chall_2, chall_3, a_tilde, in, out, params);
    }
  case 192:
    if (params->faest_param.Lke) {
      return aes_verify_192(d, qbb, chall_2, chall_3, a_tilde, in, out, params);
    } else {
      return em_verify_192(d, qbb, chall_2, chall_3, a_tilde, in, out, params);
    }
  default:
    if (params->faest_param.Lke) {
      return aes_verify_128(d, qbb, chall_2, chall_3, a_tilde, in, out, params);
    } else {
      return em_verify_128(d, qbb, chall_2, chall_3, a_tilde, in, out, params);
    }
  }
}
