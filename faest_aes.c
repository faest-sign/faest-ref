...
// static aes_128_enc_constraints_verifier(bf128_t* z_key, const bf128_t* owf_in_key, 
...
//     for (unsigned int i = 0; i < Nstbytes; i++) {
//       aes_128_add_round_key_verifier(state_bits_key, owf_in_key, k_key, params);
//     }
...
static void aes_128_constraints_verifier(bf128_t* z_key, const bf128_t* w_key, const uint8_t* owf_in, 
  ...
  if (isEM) {
    ...
    for (unsigned int i = 0; i < lambda; i++) {
      // out[i] = w[i] ^ ((owf_out[i/8] >> (i%8)) & 1);
      out_key[i] = bf128_add(w_key[i], bf128_mul(delta, bf128_from_bit(owf_out[i])));
    }
  } 
  else {
    // jump to ::13 for AES
    ...
    aes_128_expkey_constraints_verifier(z_tilde_key, k_key, w_key, delta, params);
  ...
  //aes_128_enc_constraints_verifier(z_tilde_enc_key, in_key, out_key, w_key, k_key, delta, params);
...
static void aes_192_constraints_verifier(bf192_t* z_deg0, bf192_t* z_deg1, bf192_t* z_deg2, const bf192_t* w_key, const uint8_t* owf_in, 
  ...
  //   // ::20
  //   if (b == 1) {
  //     in_key[0] = bf192_add(in_key[0], bf192_mul(delta, bf128_from_bit(1)));
  //   }
  //   // ::21
  ...
  //   aes_192_enc_constraints_verifier(z_tilde_deg0, z_tilde_deg1, z_tilde_deg2, in_key, out_key, w_key, k_key, delta, params);
