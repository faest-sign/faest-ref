...
static void aes_128_inv_norm_to_conjugates_prover(bf128_t* y_val, bf128_t* y_tag, uint8_t* x_val, bf128_t* x_tag) {
  // :1-2
...
static void aes_128_inv_norm_to_conjugates_verifier(bf128_t* y_eval, bf128_t* x_eval) {
  // :1-2
  bf128_t beta_4        = bf128_add(bf128_get_alpha(5), bf128_get_alpha(3));
  bf128_t beta_square   = beta_4;
  bf128_t beta_square_1 = bf128_mul(beta_4, beta_4);
...
static void aes_128_expkey_constraints_verifier(bf128_t* z_deg1, bf128_t* k_key, const bf128_t* w_key, 
  ...
  for (unsigned int j = 0; j < FAEST_128F_Ske / 4; j++) {
    ...
    // ::17-20
    for (unsigned int r = 0; r < 4; r++) {
      z_deg1[8*j + 2*r] = bf128_add(bf128_mul(k_hat_key_sq[r], w_hat_key[r]), bf128_mul(delta, k_hat_key[r]));
      z_deg1[8*j + 2*r + 1] = bf128_add(bf128_mul(k_hat_key[r], w_hat_key_sq[r]), bf128_mul(delta, k_hat_key[r]));
    }
    if (lambda == 192) {
      iwd += 192;
    }
...
// // ENC CSTRNTS
static void aes_128_enc_constraints_prover(bf128_t* z_deg0, bf128_t* z_deg1, bf128_t* z_deg2, const uint8_t* owf_in, const bf128_t* owf_in_tag,
    ...
    printf("starting enc constraints\n");
    /// ::1 AddRoundKey
    ...
    bf128_t* st_prime_deg2 = faest_aligned_alloc(BF128_ALIGN, 8 *  Nstbytes * sizeof(bf128_t));
    bf128_t* st_prime_deg1 = faest_aligned_alloc(BF128_ALIGN, 8 *  Nstbytes * sizeof(bf128_t));
    bf128_t* st_prime_deg0 = faest_aligned_alloc(BF128_ALIGN, 8 *  Nstbytes * sizeof(bf128_t));
    ...
    for (unsigned int r = 0; r < R/2; r++) {
      ...
      uint8_t* norms_ptr = w + 3 * Nstbits * r/2;
      bf128_t* norm_tags_ptr = w_tag + 3 * Nstbits * r/2;
      ...
      for (unsigned int i = 0; i < Nstbytes; i++) {
        ...
        for (unsigned int j = 0; j < 8; j++) {
          ...
          st_prime_deg2[i*8 + j] = bf128_mul(state_conj[conj_index], y[y_index]);
          st_prime_deg1[i*8 + j] = bf128_add(
                bf128_mul(state_conj[conj_index], y_tag[y_index]),
                bf128_mul(state_conj_tag[conj_index], y[y_index])
          );
          st_prime_deg0[i*8 + j] = bf128_mul(state_conj_tag[conj_index], y_tag[y_index]);
        }
      }
...
//       }
    ...
    faest_aligned_free(st_prime_deg2);
    faest_aligned_free(st_prime_deg1);
    faest_aligned_free(st_prime_deg0);
}
...
static void aes_128_constraints_prover(bf128_t* z_deg0, bf128_t* z_deg1, bf128_t* z_deg2, const uint8_t* w, const bf128_t* w_tag, const uint8_t* owf_in, 
  ...
  // ::7-8
  uint8_t* Rkeys = (uint8_t*)malloc(Nk*R); // storing this as uint8
  ...
  bf128_t* k_tag = (bf128_t*) malloc((R+1)*blocksize * sizeof(bf128_t));
  ...
  if (isEM) {
    ...
    constant_to_vole_128_prover(in_tag, 128);
    ...
    // aes_128_expkey_constraints_prover(z_deg1 + 1, z_deg2 + 1, k, k_tag, w, w_tag, params);
  ...
  // }
  uint8_t* w_tilde = w + Lke;
  bf128_t* w_tilde_tag = w_tag + Lke;
  bf128_t* z_tilde_deg0_ptr = z_deg0 + 1 + 2*Ske;
  bf128_t* z_tilde_deg1_ptr = z_deg1 + 1 + 2*Ske;
  bf128_t* z_tilde_deg2_ptr = z_deg2 + 1 + 2*Ske;
  ...
  aes_128_enc_constraints_prover(z_tilde_deg0_ptr, z_tilde_deg1_ptr, z_tilde_deg2_ptr, in, in_tag, owf_out, out_tag, w, w_tag, k, k_tag, params);
...
static void aes_192_constraints_prover(bf192_t* z0_tag, bf192_t* z1_val, bf192_t* z2_gamma, const uint8_t* w, const bf192_t* w_tag, const uint8_t* owf_in, 
                                        const uint8_t* owf_out, const faest_paramset_t* params, bool isEM) {
...
static void aes_128_constraints_verifier(bf128_t* z0_tag, bf128_t* z1_val, bf128_t* z2_gamma, const bf128_t* w_key, const uint8_t* owf_in, 
...
static void aes_192_constraints_verifier(bf192_t* z0_tag, bf192_t* z1_val, bf192_t* z2_gamma, const bf192_t* w_key, const uint8_t* owf_in, 
                                        const uint8_t* owf_out, bf192_t delta, const faest_paramset_t* params, bool isEM) {
                                        
}

static void aes_256_constraints_verifier(bf256_t* z0_tag, bf256_t* z1_val, bf256_t* z2_gamma, const bf256_t* w_key, const uint8_t* owf_in, 
...
static uint8_t* aes_128_verifier(const uint8_t* d, uint8_t** Q, const uint8_t* owf_in, const uint8_t* owf_out,
  ...
  aes_128_constraints_verifier(z0_tag, z1_val, z2_gamma, w_key, owf_in, owf_out, bf_delta, params, isEM);
