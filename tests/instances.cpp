...
BOOST_DATA_TEST_CASE(instance_parameters, all_parameters, param_id) {
  BOOST_TEST_CONTEXT("Parameter set: " << faest_get_param_name(param_id)) {
    ...
    const auto sig_size = (((faest_param.tau - 1) * ell_bytes)
                          + (faest_param.n_mask * ((faest_param.n_mult + 7) / 8))
                          + lambda_bytes * 4
                          + ell_bytes
                          + (faest_param.d_zk - 1) * lambda_bytes
                          + hcom_size * faest_param.tau + (faest_param.T_open * lambda_bytes)
                          + lambda_bytes
                          + IV_SIZE
                          + sizeof(uint32_t));
