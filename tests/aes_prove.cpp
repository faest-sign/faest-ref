...
BOOST_DATA_TEST_CASE(aes_prove_verify, aes_ctr_instances, param_id) {
  BOOST_TEST_CONTEXT("Parameter set: " << faest_get_param_name(param_id)) {
    ...
    std::vector<uint8_t> in(params.faest_param.beta * 16, 0);
    std::vector<uint8_t> out(params.faest_param.beta * 16, 0);
    ...
    aes_prove(w.data(), u.data(), V.data(), in.data(),
              out.data(), chall_2.data(), a_tilde.data(), b_tilde.data(),
               &params);

    uint8_t* recomputed_b_tilde = aes_verify(d.data(), Q.data(), chall_2.data(), delta.data(),
                    a_tilde.data(), in.data(), out.data(),
                    &params);
