...
namespace {
  constexpr std::array<uint8_t, 32> rootKey{0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
...
BOOST_DATA_TEST_CASE(vole_commit_verify, all_parameters, param_id) {
  BOOST_TEST_CONTEXT("Parameter set: " << faest_get_param_name(param_id)) {
    faest_paramset_t params = faest_get_paramset(param_id);
    ...
    std::vector<uint8_t*> c, v, pdec, com_j;
    ...
    voleCommit(rootKey.data(), outlen, &params, hcom.data(), vec_com.data(), c.data(), u.data(),
               v.data());
    ...
    for (uint32_t i = 0; i < params.faest_param.tau; i++) {
      ...
      uint32_t numVoleInstances = 1 << depth;
    ...
    voleReconstruct(chal.data(), pdec.data(), com_j.data(), hcomRec.data(), v.data(), outlen,
                    &params);
