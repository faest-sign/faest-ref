  ...
  std::string emit_c(const std::string& path, const std::string& name, const std::vector<Int>& f,
...

    {
      ...
      for (const auto& tbl : tables) {
        out << "extern const uint64_t " << pfx << "_" << tbl.suffix << "[" << tbl.rows_macro << "]["
            << tbl.words_macro << "];\n";
      }
      out << "extern const uint64_t " << pfx << "_TREE_MODULI[" << pfx << "_TAU];\n";
      out << "extern const uint64_t " << pfx << "_M_TREE[" << pfx << "_M_TREE_WORDS];\n\n";
      out << "#endif\n";
    }

    {
      std::ofstream out(src_path);
      check(out.good(), "failed to open source for writing: " + src_path);
      out << "/* generated by vole_mult_tables.cpp: " << name << " */\n";
      out << "#include \"" << header_name << "\"\n\n";
      for (const auto& tbl : tables) {
        out << "const uint64_t " << pfx << "_" << tbl.suffix << "[" << tbl.rows_macro << "]["
            << tbl.words_macro << "] = {\n";
      ...
      out << "const uint64_t " << pfx << "_TREE_MODULI[" << pfx << "_TAU] = { ";
      ...
      out << "const uint64_t " << pfx << "_M_TREE[" << pfx << "_M_TREE_WORDS] = { ";
